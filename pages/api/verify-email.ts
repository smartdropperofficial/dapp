...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const { data, error } = await supabase.from('email_verification_tokens').select('*').eq('token', token).limit(1).single();
        ...
        if (!user) {
            const { data: newUser, error: newUserError } = await supabase
                .from('users')
                .insert([{ email: email, email_verified: true }])
                .select()
                .single();
        ...
        const { data: newWallet, error: walletError } = await supabase
            .from('user_wallets')
            .insert([{ user_id: user.user_id, wallet_address: wallet_address }])
            .select()
            .single();
        console.log('🚀 ~ handler ~ newWallet:', newWallet);
